...
// types
import { IGptOutput, IActaConsejo } from 'src/@types/document';

// ----------------------------------------------------------------------

ActaDeConsejo.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;

// ----------------------------------------------------------------------

export default function ActaDeConsejo() {

  const [gptOutputs, setGptOutputs] = useState<IGptOutput[]>([]);
  
  const [formValues, setFormValues] = useState<IActaConsejo>();
      ...
      const fetchDocData = () => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            setFormValues(docData as IActaConsejo);
          } else {
            setError(true);
          }
        }, (error) => {
  ...
  if (formValues && !formValues.isLoading) return (
    <>
      <AddEditActaDeConsejo currentDocument={formValues} gptOutputs={gptOutputs} />
    </>
  )
}
