...
export default function CalendarForm({
  ...
  const EventSchema = Yup.object().shape({
    title: Yup.string().max(255).required('Este campo es obligatorio'),
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={3} sx={{ px: 3 }}>
        <RHFTextField name="title" label="Nombre evento" />
            ...
            <MobileDateTimePicker
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  fullWidth
                  error={!!isDateError}
                  helperText={isDateError && 'La fecha de término debe empezar después de la fecha de inicio'}
                />
              )}
            />
          )}
      ...
      <DialogActions>
        ...
        <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
          {hasEventData ? 'Actualizar' : 'Agregar'}
        </LoadingButton>
      </DialogActions>
    </FormProvider>
  );
